...
object cryptoMain extends forwardSignatures with App {
  ...
  var badKey = FastCryptographicHash(uuid).take(Ed25519.PUBLIC_KEY_SIZE)
  //Non-Secure first attempt
  for (i <- 0 until 100) {
    if (Ed25519VRF.verifyPublicKey(badKey)) {println("Key passed")} else {println("Key failed")}
    badKey = FastCryptographicHash(uuid).take(Ed25519.PUBLIC_KEY_SIZE)
  }
