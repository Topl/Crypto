...
object Slip10 {
     ...
     def derivePrivateKey(parent: ExtendedPrivateKey, index: Long): ExtendedPrivateKey = {
           ...
           val bigIntKey = new BigInteger(1,IL.bytes.toArray)
            ...
            ExtendedPrivateKey(ByteVector32(ByteVector(bigIntKey.toByteArray)), chaincode = IR, depth = parent.depth + 1, path = parent.path.derive(index), parent = fingerprint(parent))
     }
