...
import crypto.DeterministicWallet._
...
object Slip23 {
  ...
  def derivePrivateKey(parent: ExtendedPrivateKey, index: Long): ExtendedPrivateKey = {
    ...
    val I = if (isHardened(index)) {
      ...
      hmac512(parent.chaincode.bytes, publicKey(parent).publickeybytes.+:(0x02.toByte) ++ writeUInt32(index.toInt, ByteOrder.LITTLE_ENDIAN))
    }
   ...
   ExtendedPrivateKey( privChildLeftBytes32, privChildRightBytes32, chaincode = ICR, depth = parent.depth + 1, path = parent.path.derive(index), parent = fingerprint(parent))
  }
  ...
  def derivePublicKey(parent: ExtendedPublicKey, index: Long,parent_priv: ExtendedPrivateKey): ExtendedPublicKey = {
    ...
    val I = hmac512(parent.chaincode.bytes, parent.publickeybytes.+:(0x02.toByte) ++ writeUInt32(index.toInt, ByteOrder.LITTLE_ENDIAN))
    val IL = ByteVector(I.take(28).toArray)
    val IR = ByteVector32(I.takeRight(32))
    val N = BigInteger.valueOf(2).pow(256)
    ...
    val pkLeft = new Array[Byte](ec.PUBLIC_KEY_SIZE)
    val s = new Array[Byte](32)

    //pruneScalar(ChildLeftBytes32.bytes.toArray, 0, s)

    val p = new PointAccum
    scalarMultBase(ChildLeftBytes32.bytes.toArray, p)


    val pA = new PointExt
    decodePointVar(parent.publickeybytes.toArray, 0, true, pA)
    ...
    scalarMultBaseEncoded(ChildLeftBytes32.bytes.toArray, temp, 0)
    ...
    encodePoint(p, pubChildKey, 0)

    val IC = hmac512(parent.chaincode.bytes, parent.publickeybytes.+:(0x03.toByte) ++ writeUInt32(index.toInt, ByteOrder.LITTLE_ENDIAN))
