...
object Slip10Curve25519 {
    ...
    private val provider: OpportunisticCurve25519Provider = {
     ...
     def publicKey(input: ExtendedPrivateKey): ExtendedPublicKey = {
           ...
           ExtendedPublicKey(ByteVector(pubKeyByte.array()), input.chaincode, depth = input.depth, path = input.path, parent = input.parent)
     ...
     def derivePrivateKey(parent: ExtendedPrivateKey, index: Long): ExtendedPrivateKey = {
           ...
           val bigIntKey = new BigInteger(1,IL.bytes.toArray)
            ...
            ExtendedPrivateKey(ByteVector32(ByteVector(IL.bytes.toArray)), chaincode = IR, depth = parent.depth + 1, path = parent.path.derive(index), parent = fingerprint(parent))
     }
