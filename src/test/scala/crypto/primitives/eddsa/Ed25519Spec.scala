...
object Ed25519Test {
 val RANDOM = new SecureRandom
  ...
  def checkEd25519Vector(sSK: String, sPK: String, sM: String, sSig: String, text: String): Unit = {

   val RANDOM = new SecureRandom(Array(1L.toByte))


   val ec = new eddsa.Ed25519
   val sk = Hex.decode(sSK)
   val pk = Hex.decode(sPK)
   val pkGen = new Array[Byte](ec.PUBLIC_KEY_SIZE)
   ec.generatePublicKey(sk, 0, pkGen, 0)
   assert(pk sameElements  pkGen)
   val m = Hex.decode(sM)
   val sig = Hex.decode(sSig)
   val badsig = sig.clone()
   badsig(ec.SIGNATURE_SIZE - 1) = (badsig(ec.SIGNATURE_SIZE - 1) ^ 0x80).toByte
   val sigGen = new Array[Byte](ec.SIGNATURE_SIZE)
   ec.sign(sk, 0, m, 0, m.length, sigGen, 0)
   assert(sig sameElements sigGen)
   ec.sign(sk, 0, pk, 0, m, 0, m.length, sigGen, 0)
   assert(sig sameElements sigGen)
   val shouldVerify = ec.verify(sig, 0, pk, 0, m, 0, m.length)
   assert(shouldVerify)
   val shouldNotVerify = ec.verify(badsig, 0, pk, 0, m, 0, m.length)
   assert(!shouldNotVerify)
 }


  def checkEd25519ctxVector(sSK: String, sPK: String, sM: String, sCTX: String, sSig: String, text: String): Unit = {
    val sk = Hex.decode(sSK)
    val pk = Hex.decode(sPK)
    val ec = new eddsa.Ed25519
    val pkGen = new Array[Byte](ec.PUBLIC_KEY_SIZE)
    ...
    assert(pk sameElements  pkGen)
    val m = Hex.decode(sM)
    val ctx = Hex.decode(sCTX)
    val sig = Hex.decode(sSig)
    val badsig = sig.clone()
    badsig(ec.SIGNATURE_SIZE - 1) = (badsig(ec.SIGNATURE_SIZE - 1) ^ 0x80).toByte
    val sigGen = new Array[Byte](ec.SIGNATURE_SIZE)
    ec.sign(sk, 0, ctx, m, 0, m.length, sigGen, 0)
    assert(sig sameElements sigGen)
    ec.sign(sk, 0, pk, 0, ctx, m, 0, m.length, sigGen, 0)
    assert(sig sameElements sigGen)
    val shouldVerify = ec.verify(sig, 0, pk, 0, ctx, m, 0, m.length)
    assert(shouldVerify)
    val shouldNotVerify = ec.verify(badsig, 0, pk, 0, ctx, m, 0, m.length)
    assert(!shouldNotVerify)
  }
  /*
   private def checkEd25519phVector(sSK: String, sPK: String, sM: String, sCTX: String, sSig: String, text: String): Unit = {
     val sk = Hex.decode(sSK)
     val pk = Hex.decode(sPK)
     val pkGen = new Array[Byte](Ed25519.PUBLIC_KEY_SIZE)
     Ed25519.generatePublicKey(sk, 0, pkGen, 0)
     assertTrue(text, Arrays.areEqual(pk, pkGen))
     val m = Hex.decode(sM)
     val ctx = Hex.decode(sCTX)
     val sig = Hex.decode(sSig)
     val badsig = Arrays.clone(sig)
     badsig(Ed25519.SIGNATURE_SIZE - 1) ^= 0x80
     val sigGen = new Array[Byte](Ed25519.SIGNATURE_SIZE)
     val prehash = Ed25519.createPrehash
     prehash.update(m, 0, m.length)
     val ph = new Array[Byte](Ed25519.PREHASH_SIZE)
     ...
     assertTrue(text, Arrays.areEqual(sig, sigGen))
     val shouldVerify = Ed25519.verifyPrehash(sig, 0, pk, 0, ctx, ph, 0)
     assertTrue(text, shouldVerify)
     val shouldNotVerify = Ed25519.verifyPrehash(badsig, 0, pk, 0, ctx, ph, 0)
     assertFalse(text, shouldNotVerify)

     val ph = Ed25519.createPrehash
     ...
     val ph = Ed25519.createPrehash
     ph.update(m, 0, m.length)
     val shouldVerify = Ed25519.verifyPrehash(sig, 0, pk, 0, ctx, ph)
     assertTrue(text, shouldVerify)

     val ph = Ed25519.createPrehash
     ph.update(m, 0, m.length)
     val shouldNotVerify = Ed25519.verifyPrehash(badsig, 0, pk, 0, ctx, ph)
     assertFalse(text, shouldNotVerify)

   }*/
 }

 class Ed25519Spec extends FunSuite { //    @BeforeClass
  ...
  /* def setUp(): Unit = {
   ...
   test ("Test comparison Java vs. Scala") {
   ...
   test ("Test consistency"){
     val ed = new Ed25519()
     val sk = new Array[Byte](ed.SECRET_KEY_SIZE)
     val pk = new Array[Byte](ed.PUBLIC_KEY_SIZE)
     val m = new Array[Byte](255)
     val sig1 = new Array[Byte](ed.SIGNATURE_SIZE)
     val sig2 = new Array[Byte](ed.SIGNATURE_SIZE)
     Ed25519Test.RANDOM.nextBytes(m)
     for (i <- 0 until 10) {
       Ed25519Test.RANDOM.nextBytes(sk)
       ed.generatePublicKey(sk, 0, pk, 0)
       val mLen = Ed25519Test.RANDOM.nextInt & 255
       ed.sign(sk, 0, m, 0, mLen, sig1, 0)
       ed.sign(sk, 0, pk, 0, m, 0, mLen, sig2, 0)
       ...
       val shouldVerify = ed.verify(sig1, 0, pk, 0, m, 0, mLen)
       //assertTrue("Ed25519 consistent sign/verify #" + i, shouldVerify)

       assert(shouldVerify === true)

       sig1(ed.PUBLIC_KEY_SIZE - 1) = (sig1(ed.PUBLIC_KEY_SIZE - 1) ^ 0x80.toByte).toByte
       val shouldNotVerify = ed.verify(sig1, 0, pk, 0, m, 0, mLen)
 ...
 */
   ...
   test ("Check Ed25519 Tx Vector 3") {
