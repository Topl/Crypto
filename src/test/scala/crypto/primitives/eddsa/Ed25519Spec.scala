...
object Ed25519Test {
 ...
 /*private def checkEd25519Vector(sSK: String, sPK: String, sM: String, sSig: String, text: String): Unit = {
   ...
   assertTrue(text, Arrays.areEqual(sig, sigGen))
   val shouldVerify = Ed25519.verify(sig, 0, pk, 0, m, 0, m.length)
   assertTrue(text, shouldVerify)
   val shouldNotVerify = Ed25519.verify(badsig, 0, pk, 0, m, 0, m.length)
   assertFalse(text, shouldNotVerify)
 }


 private def checkEd25519ctxVector(sSK: String, sPK: String, sM: String, sCTX: String, sSig: String, text: String): Unit = {
   val sk = Hex.decode(sSK)
   val pk = Hex.decode(sPK)
   val pkGen = new Array[Byte](Ed25519.PUBLIC_KEY_SIZE)
   Ed25519.generatePublicKey(sk, 0, pkGen, 0)
   ...
   assertTrue(text, Arrays.areEqual(sig, sigGen))
   val shouldVerify = Ed25519.verify(sig, 0, pk, 0, ctx, m, 0, m.length)
   assertTrue(text, shouldVerify)
   val shouldNotVerify = Ed25519.verify(badsig, 0, pk, 0, ctx, m, 0, m.length)
   assertFalse(text, shouldNotVerify)
 }
...
/*   def testEd25519Vector1(): Unit = {
   ...
   def testEd25519Vector1023(): Unit = {
   ...
   def testEd25519ctxVector1(): Unit = {
   ...
   def testEd25519ctxVector2(): Unit = {
   ...
   def testEd25519ctxVector3(): Unit = {
   ...
   def testEd25519ctxVector4(): Unit = {
